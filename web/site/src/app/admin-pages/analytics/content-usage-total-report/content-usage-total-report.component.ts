...
@Component({
  selector: 'ss-content-usage-total-report',
  templateUrl: './content-usage-total-report.component.html',
  styleUrls: ['./content-usage-total-report.component.scss']
})
export class ContentUsageTotalReportComponent implements OnInit {
  fg: FormGroup;

  contentusagehistorytotal: AWSContentUsage[];
  cols: any[];
  ...
  ngOnInit() {
    this.getContentUsageHistoryTotal();
  }

  getContentUsageHistoryTotal() {
    this.searchByDate = false;
    this.service.getContentUsageHistoryTotal(this.fg.get('from_date').value, this.fg.get('to_date').value, this.searchByDate).subscribe((response: AWSContentUsage[]) => {
      if (response) {
        this.contentusagehistorytotal = response;
        this.contentusagehistorytotal.filter(user =>
          user.totalBytesDownloaded = this.formatBytes(user.totalBytesDownloaded, 2)
        );
        this.cols = [
          { field: 'accessedDate', header: 'Date', width: '30%' },
  ...
  onSearch() {
    ...
    this.service.getContentUsageHistoryTotal(this.fromDate, this.toDate, this.searchByDate).subscribe((response: AWSContentUsage[]) => {
      if (response) {
        this.contentusagehistorytotal = response;
        this.contentusagehistorytotal.filter(user =>
          user.totalBytesDownloaded = this.formatBytes(user.totalBytesDownloaded, 2)
