...
@Component({
  selector: 'ss-content-usage-by-user',
  templateUrl: './content-usage-by-user.component.html',
  styleUrls: ['./content-usage-by-user.component.scss']
})

export class ContentUsageByUserComponent implements OnInit {
  fg: FormGroup;
  contentusagebyusers: AWSContentUsage[];
  cols: any[];
  ...
  ngOnInit() {
    this.getContentUsageHistoryByUser();
  }

  getContentUsageHistoryByUser() {
    this.searchByDate = false;
    this.service.getContentUsageHistoryByUser(this.fg.get('from_date').value, this.fg.get('to_date').value, this.searchByDate).subscribe((response: AWSContentUsage[]) => {
      if (response) {
        this.contentusagebyusers = response;
        this.contentusagebyusers.filter(user =>
          user.totalBytesDownloaded = this.formatBytes(user.totalBytesDownloaded, 2)
        );
        this.cols = [
          { field: 'userInfo.name', header: 'Name', width: '20%' },
          { field: 'userInfo.email', header: 'Email', width: '20%' },
          { field: 'userInfo.mobileNo', header: 'Mobile No', width: '20%' },
  ...
  onSearch() {
    ...
    this.service.getContentUsageHistoryByUser(this.fromDate, this.toDate, this.searchByDate).subscribe((response: AWSContentUsage[]) => {
      if (response) {
        this.contentusagebyusers = response;
        this.contentusagebyusers.filter(user =>
          user.totalBytesDownloaded = this.formatBytes(user.totalBytesDownloaded, 2)
